...
def compute_match(
    ...
    Incorporates feedback loop: Adjusts scoring weights if past screening rejections exist in target domain.
    """
    ...
    if outcome_feedback_signals:
        for signal in outcome_feedback_signals:
            ...
            if job_domain in pattern or "screening_rejection" in pattern:
                # Heuristic: past rejection in this domain indicates missing target ATS keywords
                domain_penalty = 12.0
    ...
    result = {
        ...
        "is_qualified": composite_score >= 65.0
    }
    if feedback_notes:
