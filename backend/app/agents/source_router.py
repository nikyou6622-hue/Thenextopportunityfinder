"""
source_router.py — Agent 5: Smart ATS Application Classifier & Link Resolver (app/agents)
...
def resolve_and_validate_apply_url(
    ...
    req_timeout = (1.5, min(3.0, float(timeout_sec)))
    try:
        # First attempt lightweight HEAD request
        resp = http_client.head(raw_norm, headers=headers, allow_redirects=True, timeout=req_timeout)
        ...
        if resp.status_code in [405, 403, 400]:
            try:
                get_resp = http_client.get(raw_norm, headers=headers, allow_redirects=True, timeout=req_timeout, stream=True)
...
def revalidate_job_links(db: Any, max_age_hours: int = 72, limit: int = 100) -> Dict[str, Any]:
    ...
    def _check_job_url(job_data):
        job_id, raw_url = job_data
        if not raw_url:
            return job_id, "", "unchecked"
        resolved_url, status = resolve_and_validate_apply_url(
            raw_url,
            timeout_sec=2.5,
            check_live=True
        )
        return job_id, resolved_url, status

    try:
        job_map = {job.id: job for job in stale_jobs}
        job_tuples = [(job.id, job.apply_url_raw or job.apply_url) for job in stale_jobs]

        with ThreadPoolExecutor(max_workers=15) as executor:
            future_to_id = {executor.submit(_check_job_url, item): item[0] for item in job_tuples}
            for future in as_completed(future_to_id):
                ...
                if not job:
                    continue
                try:
                    _, resolved_url, status = future.result()
                    ...
                    logger.warning(f"Revalidation failed for job {job_id}: {ex}")
                    summary["unchecked_count"] += 1
