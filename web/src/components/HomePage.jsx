...
import { CinematicFooter } from './ui/motion-footer.tsx';
...
const WORKFLOW_STEPS = [
  {
    ...
    badge: 'Step 1: The Foundation',
    color: '#6366f1',
    icon: FileText,
    targetTab: 'profile',
    description: 'Import your existing PDF/DOCX resume or seed verified sample data. Edit directly on a live A4 sheet using 11 certified ATS templates (including FAANG Tech, Corporate HR, Clinical Pro, GAAP Finance, and UI/UX Designer) with real-time 5-pillar scoring and page-overflow warnings.',
  ...
  },
  {
    ...
    badge: 'Step 2: Opportunity Matching',
    color: '#38bdf8',
    icon: Search,
    targetTab: 'jobs',
    description: 'Explore live, active technical opportunities matching your exact skills across Indian product unicorns (Swiggy, Razorpay, CRED), verified Indian internships, and direct multinational career portals (Google, Microsoft, Amazon, Meta). Automated link health checking ensures 0% ghost postings.',
  ...
  },
  {
    ...
    badge: 'Step 3: Maximize Callbacks',
    color: '#ec4899',
    icon: Sparkles,
    targetTab: 'tailor',
    description: 'Select any target opportunity to generate a job-specific customized CV. The engine identifies missing technical keywords and aligns your verified experience to the job requisition without ever fabricating past companies, degrees, or unearned metrics.',
  ...
  },
  {
    ...
    badge: 'Step 4: Ace the Interview',
    color: '#10b981',
    icon: BrainCircuit,
    targetTab: 'interview-prep',
    description: 'Simulate high-pressure technical and behavioral interviews with our real-time voice interviewer. Answers are transcribed live and evaluated against the STAR framework (Situation, Task, Action, Result) with company-specific question sets and actionable feedback.',
  ...
  },
  {
    ...
    badge: 'Step 5: Code & Connect',
    color: '#06b6d4',
    icon: Code,
    targetTab: 'coding',
    description: 'Sharpen your coding skills in our in-browser Python, JavaScript, and SQL Coding Prep studio with live test runner. Generate hyper-personalized cold outreach emails targeting hiring managers and engineering leads to bypass traditional application queues.',
...
const FAQS = [
  {
    q: 'What is Thenextopportunity and how is it different from traditional job boards?',
    a: 'Thenextopportunity is an end-to-end AI career operating system. Unlike simple aggregators, it combines live opportunity discovery (with verified links and direct apply portals) with a real-time ATS live resume studio (11 templates), 1-click tailored CV generation, voice-powered AI mock interviews, an in-browser DSA coding sandbox, and recruiter outreach.'
  },
  {
  ...
  },
  {
    ...
    a: 'Thenextopportunity is built with privacy by design. We support end-to-end field-level encryption, user data portability (JSON export), and the Right to Erasure in our Settings & Privacy Hub. Your data is never sold to third-party data brokers.'
  }
];
...
export default function HomePage({ onNavigate, currentUser, onTriggerCelebration, onOpenPaywall, isPro = false }) {
  ...
  const simScore = Math.min(99, Math.max(40, Math.round(
    (simSkillsCount * 3.8) +
    (simHasMetrics ? 25 : 8) +
    (simTemplate === 'ats_safe' || simTemplate === 'modern' ? 18 : 14) +
    18
  )));
  ...
  const toggleChecklistItem = (key) => {
    ...
    setChecklist(prev => {
      ...
      if (!prev[key] && onTriggerCelebration) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '36px', maxWidth: '1400px', margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <img 
            ...
            style={{ width: '24px', height: '24px', objectFit: 'cover', borderRadius: '50%', background: 'transparent', padding: '0' }} 
          />
          ...
          <span style={{ fontSize: '0.78rem', color: '#cbd5e1', fontWeight: 600 }}>
            Next Opportunity Finder OS <strong style={{ color: '#fff' }}>v2.14.0 Production Build</strong> — Find Your Next Step &bull; {isPro ? 'PRO LIFETIME UNLOCKED' : '5 Free Scrapes Available'}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {currentUser ? (
            ...
            >
              <Rocket size={14} /> Go to Live Dashboard ({currentUser.full_name?.split(' ')[0] || 'User'}) →
            </button>
          ) : (
      ...
      {/* 🌟 2. HERO THESIS BANNER WITH NOVA MASCOT */}
      <div className="glass-panel" style={{
        padding: '36px 32px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '820px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', background: 'rgba(99, 102, 241, 0.2)', border: '1px solid rgba(99, 102, 241, 0.4)', padding: '6px 16px', borderRadius: '24px', alignSelf: 'flex-start' }}>
              ...
              <span style={{ fontSize: '0.8rem', fontWeight: 800, color: '#A5B4FC', letterSpacing: '0.04em' }}>
                NEXT OPPORTUNITY FINDER &bull; FIND YOUR NEXT STEP
              </span>
            </div>
            ...
            return (
              <div 
                ...
                style={{
                  ...
                  boxShadow: `0 8px 24px rgba(0, 0, 0, 0.4), 0 0 16px ${step.color}10`,
                  position: 'relative'
                }}
              >
                {/* Step Number & Badge */}
      ...
      {/* 🌟 5. MEET YOUR ORIGINAL CAREER CREW (NOVA, LEXI, ZENITH, PIXEL) */}
      <div className="glass-panel" style={{
        padding: '32px 28px',
      ...
      <div id="ats-section" className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {['fullstack', 'aiml', 'devops'].map((role) => (
              <button
                key={role}
                ...
                style={{
                  ...
                  fontWeight: 700,
                  cursor: 'pointer',
                  background: simRole === role ? '#6366f1' : 'rgba(255, 255, 255, 0.05)',
                  color: simRole === role ? '#ffffff' : '#94a3b8'
                }}
              >
                {role === 'fullstack' ? 'Full Stack' : role === 'aiml' ? 'AI / ML' : 'DevOps'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: '#cbd5e1', marginBottom: '6px' }}>
                <span>Technical Skills Count:</span>
                <strong style={{ color: '#818cf8' }}>{simSkillsCount} Skills</strong>
              </div>
              <input 
                type="range" 
                min="2" 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255, 255, 255, 0.03)', padding: '10px 14px', borderRadius: '10px' }}>
              <span style={{ fontSize: '0.8rem', color: '#cbd5e1' }}>Quantifiable Metrics (% / Impact) Included:</span>
              <button
                onClick={() => setSimHasMetrics(!simHasMetrics)}
                style={{
                  padding: '4px 10px',
                  borderRadius: '6px',
                  border: 'none',
          ...
          <div style={{ background: 'rgba(20, 26, 48, 0.6)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '16px', padding: '18px', textAlign: 'center' }}>
            <div style={{ fontSize: '0.74rem', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Projected ATS Score
            </div>
            <div style={{ fontSize: '2.6rem', fontWeight: 900, color: simScore >= 80 ? '#34d399' : (simScore >= 60 ? '#60a5fa' : '#fbbf24'), margin: '4px 0' }}>
              {simScore} <span style={{ fontSize: '1rem', color: '#94a3b8' }}>/ 100</span>
            </div>
            <div style={{ fontSize: '0.76rem', color: '#cbd5e1' }}>
              {simScore >= 80 ? '🔥 High ATS Pass Rate (>85%)' : '⚠️ Action required: add metrics & keywords'}
            </div>
          </div>
        </div>
      </div>
